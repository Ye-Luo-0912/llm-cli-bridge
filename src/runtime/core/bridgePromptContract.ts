...
export function buildCapabilityManifest(
  ...
  lines.push("- vault-api Skill: .claude/skills/vault-api/SKILL.md（物化后）。只暴露文件系统做不到的能力：property_get/set（frontmatter）、tags_list、backlinks_get、tasks_list、daily_read/append、outlinks_get、broken_links_list、headings_get（metadataCache 解析）、search（markdown-aware 全文搜索，跳过 frontmatter/代码块）、metadatacache_get（单文件完整 cache 聚合）、bookmarks_list（读 .obsidian 书签）、vault_delete/rename/restore + rename_tag（走回收站/原子改 frontmatter）。调用通道：HTTP bridge（读 .llm-bridge/bridge.json 取 port/token）→ helper mjs → outbox actions.jsonl 兜底。普通文件读写仍用 native file tools。");
