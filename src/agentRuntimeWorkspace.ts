...
export function generateInitialVaultApiSkill(): string {
  return [
    ...
    "- 全 vault 标签/反向链接/待办清单/出链/断链/标题大纲 → **必须**用本 Skill（文件系统无法高效反查或做不准解析）。",
    "- daily note → 用本 Skill（自动解析 daily-notes 插件配置的日期格式与目录）。",
    "- 删除/重命名/恢复 → 用本 Skill（走回收站、更新 metadataCache；不要直接 fs.unlink/rename）。",
    "- path 参数必须是 vault 相对路径（如 `inbox/note.md`），禁止绝对路径与 `..` 遍历。",
    "- 修改类 action（property_set/daily_append/vault_delete/vault_rename/vault_restore）会弹审批框，用户拒绝则不执行。",
    "- 13 个 action 之外的 Obsidian 能力暂未暴露；如需扩展请在 LLM-AgentRuntime/skills/vault-api/SKILL.md 记录需求。",
    "",
  ].join("\n");
}
