...
- **测试时间**: 2026-07-13T08:32:40.649Z
- **测试环境**: win32 / Node.js v24.14.0
- **插件版本**: 2.18.0
- **main.js 大小**: 1392.9 KB
...
- **运行命令**: node scripts/run-tests.mjs --unit --audit
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 最小三类 section，不堆砌细则 | allPresent=true hasHeaders=true reasonableLen=true capLen=1631 autoLen=195 safetyLen=326 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | reads plugin skills/SKILL.md | skills=[{"id":"pdf@openai-primary-runtime:pdf","name":"pdf","description":"Read and verify PDF files.","skillPath":"D:\\Users\\Ye_Luo\\APP\\Test\\llm-cli-bridge\\.test-managed-plugin-skills-SbEHPv\\skills\\pdf\\SKILL.md"}] |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | session 声明在 buildRuntimeCapabilities 之前 | sessionLine=19920 capLine=20680 orderOk=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | V3 结构 SKILL.md frontmatter + references/ + agents/openai.yaml | frontmatter=true routerVer=true subSkills=true maintenance=true routing=true vaultRulesHasBoundary=true directoriesHasSemantics=true openaiYaml=true |

### V16.5-E generateInitialVaultSkill

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | V3 结构 skillMd + references(4) + agentsOpenaiYaml + indexMd | isObject=true skillMdOk=true referencesOk=true keys=["conventions.md","directories.md","preferences.md","vault-rules.md"] vaultRulesOk=true directoriesOk=true agentsYamlOk=true indexMdOk=true |

### VC-MIGRATE

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 旧版 v1 SKILL.md 迁移到 V3 references/ 分发（中文偏好→references/preferences.md） | skillIsRouting=true rewrittenOk=true prefsHasChinese=true prefsMigrated=true rulesHasBoundary=true dirsHasInbox=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 包含 Agent workspace / Vault Skill 包 / V3 references 上下文分区 事实 | agentWs=true vaultSkillPkg=true references=true facts=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | references/ 子文件超 15 条 compact 到 15，SKILL.md 不动（轻量版无 split） | action=compacted bulletCount=15 noSplit=true rulesCapped=true skillMdUntouched=true structureDirExists=false indexRegenerated=true |

### V16.5-K 轻量版

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 多 references/ 子文件 compact 不 split | action=compacted noSplit=true allCapped=true counts=[15,15,15,15] structureExists=false |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | vault-context SKILL.md frontmatter + 上下文分区路由（无 split） | vcLen=1356 underMax=true splitNotice=false indexPointer=false frontmatter=true routerVer=true subSkills=true maintenance=true routesToRefs=true |
...
### V3 materializeAgentSkillToTarget

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | sourceDir 时递归复制 references/ 到目标目录 | ok=true status=created subA=true subB=true subAContent=true skillMd=true |
...
| ✅ | 缺失时回退到 toolName 关联 id 保持一致 | start=pi-sdk-read-1783931562718-0 update=pi-sdk-read-1783931562718-0 end=pi-sdk-read-1783931562718-0 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | --wait --timeout 超时行为（fake server） | exit=1 elapsed=3247ms hasTimeout=true hasAssertion=false stderr=等待超时（2s）。actionId: timeout-test-id
 |
| ✅ | --wait 成功路径（fake server 第 3 次轮询转 completed） | exit=0 elapsed=4689ms hasCompleted=true stdout=Action 已完成。actionId: fake-id-1783931568536
 |
| ✅ | health 命令（fake server） | - |
| ✅ | --json 标志输出有效 JSON（fake server） | - |
| ✅ | 非修改类 action 直接输出（不轮询） | {
  "ok": true,
  "id": "fake-id-1783931573411",
  "status": "completed",
  "result": {
    "type":  |
| ✅ | --stdin 模式读取 JSON params | {
  "ok": true,
  "id": "fake-id-1783931573527",
  "status": "completed",
  "result": {
    "type":  |
| ✅ | --raw 输出纯 JSON（单行） | {"ok":true,"id":"fake-id-1783931573635","status":"completed","result":{"type":"tags_list","fake":tru |
...
| ✅ | 步骤用户导向（含打开笔记/总结当前笔记，不含 Backend/Preflight） | - |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 返回非空 id | id=s-2026-07-13T08-33-20-447Z-feu4fo |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 按 savedAt 降序（最新在前） | len=5 first=s-2026-07-13T08-33-20-523Z-8o7u0c second=s-2026-07-13T08-33-20-457Z-8s7q57 |
| ✅ | 空目录返回空数组 | len=0 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 生成 s- 前缀且唯一 | id1=s-2026-07-13T08-33-20-595Z-u05vbl id2=s-2026-07-13T08-33-20-595Z-gwynap |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | applyCount+1 且 lastUsedAt 更新 | before=0 after=1 lastUsedAt=2026-07-13T08:33:20.683Z |
...
| ✅ | savedAt 非字符串用当前时间 | savedAt=2026-07-13T08:33:20.771Z |
...
| ✅ | savedAt 更新为当前时间 | before=2026-07-13T08:33:20.874Z after=2026-07-13T08:33:20.940Z |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 重命名后新名 meta 完整 + 旧名孤儿清理 | newOk=true oldGone=true oldFileGone=true newFileExists=true newMeta={"applyCount":3,"lastUsedAt":"2026-07-13T08:33:22.188Z","pinned":true,"groupOverride":"测试组"} |

### V2.12.1 字段完整性

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | pinned/applyCount/lastUsedAt/groupOverride 全部迁移 | pinned=true applyCount=5 lastUsedAt=2026-07-13T08:33:22.197Z groupOverride=GroupA oldGone=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 物化到 Codex home personal skills 而非 .claude | path=C:\Users\Ye_Luo\AppData\Local\Temp\llm-bridge-codex-home-JbXaWP\skills\llm-bridge-78ae3e1b-review-skill\SKILL.md |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ⏭️ | V2.14.0-I1 symlink realpath hardening runtime test | 当前环境无法创建 symlink/junction: EPERM: operation not permitted, symlink 'C:\Users\Ye_Luo\AppData\Local\Temp\llm-bridge-i1-external-1qvB3R\outside.md' -> 'C:\Users\Ye_Luo\AppData\Local\Temp\llm-bridge-i1-vault-TqYWP6\link-out.md' |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ⏭️ | V2.14.0-J route symlink escape runtime test | 当前环境无法创建 symlink；静态确认路由委托 executor realpath guard=true: EPERM: operation not permitted, symlink 'C:\Users\Ye_Luo\AppData\Local\Temp\llm-bridge-j-external-Mn5Ir3\outside.md' -> 'C:\Users\Ye_Luo\AppData\Local\Temp\llm-bridge-j-vault-RDgJKH\link-out.md' |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | process head 可折叠（is-collapsible/role=button/aria-expanded + click 切换） | collapsible=true role=true expanded=true toggleExp=true -> collapsed: aria=true toggle=true -> expanded: aria=true toggle=true |
| ✅ | formatQuietProcessedLabel 产出 'Processed · {elapsed}'（无 cluster title 时渲染 quiet label） | quietLabel=Processed · 3.2s |

### V17-G56 契约

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | mountOrReconcileCodexRun 导出为函数（mount + patch 入口） | - |

### V17-G56 CSS 契约

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | process-toggle/section-title/final-answer/session/history/modal 选择器存在 | - |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 最终回答独立兄弟节点 + 原地升级 Markdown 不重建节点（结构） | - |

### Phase 1.4-GUARD-3

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | command 1→2 条时 cluster key 不变（结构） | - |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 真实 segmentCodexFeedEntries key 稳定性 + reconciliation（生产函数） | keysMatch=true stable=true sameA=true sameB=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 真实 segmentCodexFeedEntries command 1→2 条 cluster key 不变（生产函数） | isCluster=true sameKey=true oneCluster=true key=cluster:execution:seq:1:cmd1 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 真实 reconcileCodexRunWaterfall 最终回答不在 process body 内（生产函数） | feedList=true candidateInFeed=0 answerBodyInFeed=0 section=true sectionOutsideProcess=true answerInSection=true mdOnce=true mdDedup=true streamInSection=true answerBodyDuringStream=0 candidateInFeedStream=0 answerBodyInFeedStream=0 |

### Phase 3-DOM-12

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 真实 cluster 首次点击懒加载 + 连续开合复用 body（生产函数） | groupExists=true initiallyClosed=true chevronExists=true noBodyBeforeToggle=true toggleFired=true bodyCreated=true members=2 chevronExpanded=true sameGroup=true bodyReused=true bodyReusedAfterReopen=true renderCalls=2 |

### VC-SKILL

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | V3 metadata 触发 + 按需路由 + 不再每轮注入 prompt | metadata=true routing=true noRetriever=true noPromptInjection=true |

### VC-CSS

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 间距变量存在 + thinking-line/feed-batch/feed-list 收敛（legacy+run-view 各 1 定义） | gapThinking=true gapToolInternal=true gapStage=true guideBorder=true usesVar=true thinkingLineDefs=2 feedBatchDefs=2 feedListDefs=2 |
...
|------|--------|------|
| ✅ | onOpen() 无异常 | - |
| ✅ | 状态不再停留在初始化 | statusText="Test Runtime · 可用" |
| ✅ | 模型 chip 按钮存在 | modelChip=found |
| ✅ | 发送按钮存在 | sendBtn=found |
| ✅ | 文件 input 存在且 type=file | fileInput=found |
| ✅ | 点击 + 按钮触发文件选择器 | attachBtn=found, clickCalled=true |
| ✅ | composer 基础 DOM 存在（tools-left + composer-bar） | toolsLeft=found, composerBar=found |
| ✅ | 事件绑定/数据填充分离 + 重试幂等 + setGlobalStatus | bindEvents=true, noEventRegInHydrate=true, noBareStatus=true, setStatus=true |
...
| ✅ | 不再持久化 pendingModels（已移除模型发现） | - |
| ✅ | 不再持久化 incompatibleModels | noIncompatibleModels=true |
| ✅ | 有 router.migrateFromV20_4 迁移按钮 + setActiveProvider（替代 discoveredAt） | migrate=true setActive=true noDiscoveredAt=true |

### V20.7 settings.ts

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 使用 tab + saveProviderForm + readProviderForm | tabs=true, saveForm=true, readForm=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 认证失败关键词全部检测到 (401/403/unauthorized/api key/auth/invalid key) | detected=8/8 |
| ✅ | 非认证错误不误判为认证失败 | 误判:  |
| ✅ | agentRunDisplayModel 含 isAuthError 检测 + 'API Key 缺失或无效' 标题 | isAuthError=true, title=true, regex=true |
| ✅ | .llm-bridge-init-error-card 中性底色 + 红色边框 | bgNeutral=true, borderRed=true, bg="var(--background-secondary)" |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | API Key 通过 saveProviderForm + clearXxxKey 持久化 | - |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | send timeoutMs 超时后 reject | rejected=true, elapsed=210ms |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | refreshDynamicModelCatalog 使用本地配置模型 | localModel=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | codex 本地配置存在 → CODEX_HOME 设置 | CODEX_HOME=C:\Users\Ye_Luo\AppData\Local\Temp\v205-test-i8R2RC\.llm-bridge\private\runtime\codex, hasKey=true |
| ✅ | codex 本地配置缺失 → CODEX_HOME 不设置 | CODEX_HOME=undefined, hasKey=true |
| ✅ | claude 本地配置存在 → CLAUDE_CONFIG_DIR 设置 | CLAUDE_CONFIG_DIR=C:\Users\Ye_Luo\AppData\Local\Temp\v205-test-x5ihma\.llm-bridge\private\runtime\claude, hasKey=true |
| ✅ | pi 本地配置存在 → PI_CODING_AGENT_DIR 设置 | PI_CODING_AGENT_DIR=C:\Users\Ye_Luo\AppData\Local\Temp\v205-test-YelEaD\.llm-bridge\private\runtime\pi, hasKey=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 旧 legacyApiKey 仅迁移给 codex，Claude/Pi 不被自动设置 key | codex=true, claude=false, pi=false |
