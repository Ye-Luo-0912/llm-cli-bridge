...
| ✅ | bridgeSystemAppend → instructions, userPrompt → turn/start input | - |
...
| ❌ | cwd 不存在 → failed diagnostic | cwdExists=true, available=true, diag="[preflight] profile: custom
command: node
cwd: ok
status: available
version: v24.15.0" |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ❌ | ClaudeCliBackend 不产生 workflow 事件 | hasStdout=false noWfEvents=true wfCount=0 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ❌ | ClaudeCliBackend 不产生 workflow 事件（V1.7 验证） | hasStdout=false noWfEvents=true wfCount=0 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ❌ | auto 模式正常产出 stdout | hasStdout=false |
