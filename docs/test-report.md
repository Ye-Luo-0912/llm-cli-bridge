...
- **bridge.json 存在**: 否
- **HTTP 端口**: N/A
- **commit sha**: 9a133206d68654e36c9e5b90c214dbe11062ad3a
- **commit 短 sha**: 9a133206d686
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | session 声明在 buildRuntimeCapabilities 之前 | sessionLine=291281 capLine=291424 orderOk=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | pi-sdk probe 不抛错 + reason/available 语义一致 | reason=installed available=true providerAvailable=true |
...
| ✅ | 缺失时回退到 toolName 关联 id 保持一致 | start=pi-sdk-read-1783243561267-0 update=pi-sdk-read-1783243561267-0 end=pi-sdk-read-1783243561267-0 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 未安装时返回可行动提示 | hasAuth=false hasModel=false hint=Pi SDK 已安装但未加载（ESM-only）。请通过插件面板触发运行时加载，或重启 Obsidian。 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | pi-sdk 是 portable 主线候选（providerId=pi-sdk） | providerId=pi-sdk available=true |
...
| ✅ | 无 auth/model 时返回可行动提示 | hasAuth=false hasModel=false hint=Pi SDK 未配置认证和模型。请在 ~/.pi/agent 配置 API Key 或运行 pi login，并在插件设置中选择 model。 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | V17-C smoke:pi-sdk: read-only + pi-native 两组 + friendReady gate + skip 明确 | readOnly=true piNative=true friendGate=true skip=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | V17-C1 smoke:pi-sdk 状态字段完整（piSdkSmokeStatus/piReadOnlySmokeStatus/piNativeSmokeStatus/friendReady/skip reason） | piSdk=true readOnly=true piNative=true friendReady=true skip=true passGate=false |
...
### V17-C1 enable-friend-preview

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 显式设 backendMode=auto（preset 完整） | backendModeAuto=true profilePortable=true toolModePiNative=true trustFalse=true |

### V17-C1 tryLoadPiSdkAsync

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 导出 + run() async 补齐 + isPiSdkPackageInstalled + importModule（ESM-only SDK 加载） | asyncExport=true runUsesAsync=true isInstalled=true importModule=true |

### V17-C1 view.ts hint card

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 用 tryLoadPiSdkAsync（非 sync tryLoadPiSdk） | usesAsync=true notSyncOnly=true |

### V17-C1 release bundle

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 不含 node_modules + installer 策略完整（installHint + run 提示 + view 提示） | noNodeModules=true installHint=true runReportsInstall=true viewShowsInstall=true |

### V17-C1 getActiveToolNames fixture

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | bridge-controlled 工具集 = read + bridge_* （任务 E 真实 SDK fixture） | dev: read=true bw=true be=true bb=true noBuiltin=true | portable: read=true bw=true be=true activeCount=7 |

### V17-C1 tryLoadPiSdkAsync integration

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 接口契约正确（导出 + sync shape + auth probe 不抛错） | async=true sync=true cacheClear=true authProbe=true syncShape=true syncReason=installed |

### V17-C1 回归 materialize all targets

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 命令存在 + portable 目标 + 全量物化 | cmd=true portableTargets=true allTargets=true |

### V17-C1 回归 Claude/Codex

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | developer 链路（codex→sdk→cli）+ portable 链路（pi-sdk→pi-rpc）并存 | codex=true sdk=true cli=true portableChain=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 返回非空 id | id=s-2026-07-05T09-26-15-594Z-poq6rt |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 按 savedAt 降序（最新在前） | len=5 first=s-2026-07-05T09-26-15-650Z-93bi2u second=s-2026-07-05T09-26-15-598Z-8sj7c1 |
| ✅ | 空目录返回空数组 | len=0 |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 生成 s- 前缀且唯一 | id1=s-2026-07-05T09-26-15-657Z-5cwb1x id2=s-2026-07-05T09-26-15-657Z-p5z15t |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | applyCount+1 且 lastUsedAt 更新 | before=0 after=1 lastUsedAt=2026-07-05T09:26:15.682Z |
...
| ✅ | savedAt 非字符串用当前时间 | savedAt=2026-07-05T09:26:15.710Z |
...
| ✅ | savedAt 更新为当前时间 | before=2026-07-05T09:26:15.745Z after=2026-07-05T09:26:15.796Z |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | 重命名后新名 meta 完整 + 旧名孤儿清理 | newOk=true oldGone=true oldFileGone=true newFileExists=true newMeta={"applyCount":3,"lastUsedAt":"2026-07-05T09:26:16.994Z","pinned":true,"groupOverride":"测试组"} |

### V2.12.1 字段完整性

| 状态 | 测试项 | 详情 |
|------|--------|------|
| ✅ | pinned/applyCount/lastUsedAt/groupOverride 全部迁移 | pinned=true applyCount=5 lastUsedAt=2026-07-05T09:26:16.997Z groupOverride=GroupA oldGone=true |
...
| 状态 | 测试项 | 详情 |
|------|--------|------|
| ⏭️ | claude 可用性 | claude 不可用 (exitCode=127)；diag: exit code: 127 |
