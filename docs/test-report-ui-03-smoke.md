...
| Check | Status | Detail |
| --- | --- | --- |
| UI-03 左 rail nav-label: 4 个导航项均有 label span | pass | ok |
...
| UI-03 Skills 分组: 本轮已启用 vs 可用但未启用 | pass | ok |
| UI-03 Skills 分组标签: '本轮已启用（N）' + '可用但未启用（N）' | pass | ok |
| UI-03 renderAgentSkillItem: 抽取为独立方法（AgentSkillRecord 类型） | pass | ok |
| UI-03 CSS Skills 分组: uppercase + accent 色标签 | pass | ok |
| UI-03 恢复提示: 消息数 + 模型 + 权限 + Pin + 原生会话 | pass | ok |
| UI-03 Files 三段: 本轮上下文 / Pin / 外部读取请求（已有，验证保留） | pass | ok |
| UI-03 顶栏: 会话标题 + 新聊天 + 设置 + runtime 状态（已有，验证保留） | pass | ok |
| UI-03 nav-item tooltip/aria-label: 保留（键盘可访问） | pass | ok |
