#!/usr/bin/env node
// V2.15-G Final RC2 Smoke — CDP 自动化最终检查
...
const CDP_PORT = 9223;
const REPORT_PATH = path.resolve(PLUGIN_DIR, 'docs', 'V2.15-G_FINAL_RC2_SMOKE.md');
...
async function check5_attachmentMenu(client) {
  const js = `(() => {
    try {
      ...
      const items = root.querySelectorAll('.llm-bridge-attachment-menu-item');
      const bodyItems = root.querySelectorAll('.llm-bridge-attachment-menu-body .llm-bridge-attachment-menu-item');
      return {
        menuFound: !!menu,
        itemCount: items.length,
        bodyItemCount: bodyItems.length,
  ...
  const v = res.result.value;
  if (v.error) return { pass: false, reason: v.error, details: v };
  const pass = v.menuFound && v.itemCount === 4;
  return {
    pass,
    reason: pass ? "" : `menuFound=${v.menuFound} itemCount=${v.itemCount} (期望 4)`,
    details: v,
  };
}
...
function generateReport(d) {
  ...
  return `# V2.15-G Final RC2 Smoke Report
...
### 5. AttachmentMenu 四项: ${r5.pass ? "✅ PASS" : "❌ FAIL"}
- ${fmtCheck(r5.details?.menuFound, "")} .llm-bridge-attachment-menu 存在 (tag=${r5.details?.menuTag})
- ${fmtCheck(r5.details?.itemCount === 4, "")} .llm-bridge-attachment-menu-item 数量 = ${r5.details?.itemCount} (期望 4)
...
## Recommendation
${overallPass ? "✅ V2.15.0 RC2 最终 smoke 全部通过，建议作为正式发布基线（efd080a）。UI 行为（tab 切换、Agent Skills only、composer 输入、attachment menu、model/effort picker）与代码级边界（Claude Code/SDK minimal prompt、native read/edit、external write deny、sensitive .env deny）均验证通过。" : "❌ V2.15.0 RC2 最终 smoke 存在失败项，需排查后再发布。失败项见上文 ❌ 标记。"}
...
// ============================================================
async function main() {
  console.log("[V2.15-G Final RC2 Smoke] 开始...");
  ...
  const allPass = results.check1.pass && results.check2.pass && results.check3.pass && results.check4.pass && results.check5.pass && results.check6.pass && results.check7.pass && results.check8.pass && results.check9.pass && results.check10.pass && results.check11.pass;
  client.close();
  ...
  console.log(`\n=== V2.15-G Final RC2 Smoke: ${allPass ? "PASS" : "FAIL"} ===`);
