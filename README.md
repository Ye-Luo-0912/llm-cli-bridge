...
1. **安装并启用插件** — 确认本地 `claude --version` 可用，启用插件后点 Preflight 检测（绿色即可用）
2. **打开笔记或选中文字** — 插件自动识别当前笔记和选区（底部 chips 显示状态）
3. **点底部按钮** — 「总结当前笔记」/「解释选区」/「自由提问」，再点 ↑ 或 Ctrl/Cmd+Enter 发送

> 日常使用只需默认 auto 模式，无需理解 backend / SDK / mock 等技术细节。
